    ...
    class TrackingOrderActivity : BaseActivity() {
        ...
        private fun drawRoute(shopLocation: LatLng, adminAddress: String) {
            ...
            service.getDirections(
                "${latitudeData},${longitudeData}", // Customer location
                "${shopLocation.latitude},${shopLocation.longitude}", // Shop location
                apiKey
                    ...
                    if (directionResponse != null && directionResponse.routes.isNotEmpty()) {
                        ...
                        googleMap.addPolyline(
                            PolylineOptions().addAll(decodedPath)
                                .color(ContextCompat.getColor(this@TrackingOrderActivity, R.color.colorPrimary))
                                .width(18f)
                        )
                        googleMap.addMarker(
                            MarkerOptions()
                                ...
                                .title("Customer")
                                .snippet(mOrder!!.address!!.address)
                                .icon(
        ...
        private fun sendNotificationToToken(token: String, orderId: String, userId: String) {
            val data = hashMapOf(
                "title" to (" 🎉 " + "Đơn hàng đã bị hủy"), "body" to "Đơn hàng $orderId đã bị hủy"
            )
                ...
                .enqueue(object : Callback<Notification> {
                    override fun onResponse(
                        ...
                        if (response.isSuccessful) {
                            ...
                            Toast.makeText(
                                this@TrackingOrderActivity, "Notification failed", Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
        ...
        private fun getDeviceTokenForUser(userId: String, onComplete: (String?) -> Unit) {
            ...
            database.child(userId).get().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    val token =
                        task.result.children.firstOrNull()?.child("token")?.getValue(String::class.java)
                    onComplete(token)
                } else {
